...
export class TagObsoleteKeysCommand implements Command {
	private readonly obsoleteKeyTag: string;
	...
	public async run(): Promise<void> {
		...
		const result = await this.tmsClient.tagKeys(obsoleteKeys, this.obsoleteKeyTag);

		if (result.errors.length) {
			warning(`Error while tagging obsolete keys`);
		}

		info(`Obsolete keys tagged with ${this.obsoleteKeyTag}`);
	}
